...
export const kleur_onbekend = "#f0e442";
const kleur_ja = "#009e73";
const kleur_nee = "#cc79a7";
const kleur_geen_gegevens = "#7774";
...
export function createGemeenteStyleFunction(
  ...
  return (feature: FeatureLike, _resolution: number): Style => {
    ...
    const { totaal, bovenPercentage } = isBovenPercentage(data.value, gem, tg.value, percentage.value);
    let kleur: string;
    ...
    if (bovenPercentage) {
      ...
      if (data.value[gem][1] === GEEN_VERKIEZING) {
        ...
        kleur = data.value[gem][2]
            ? (data.value[gem][2][tg.value]?.n === totaal ? kleur_nee : kleur_onbekend)
            : kleur_geen_gegevens;
      }
    } else {
      kleur = kleur_geen_gegevens;
    }
    return new Style({
      fill: new Fill({
        color: kleur,
