...
async function tekenGrafiek() {
  if (
    ...
    (ingelezenVerkiezing.value != props.informatie.verkiezing &&
      props.informatie.verkiezing != null)
  ) {
    ingelezenVerkiezing.value = props.informatie.verkiezing;
...
<template>
  <div class="midden">
    <template v-if="props.informatie.verkiezing != 'ep2024'">
      <h3>Aantal gemeenten die gegevens hebben gepubliceerd</h3>
      <div style="margin-left: 10px" ref="ggGrafiek"></div>
      <h3 v-if="ontbrekendeGemeenten?.length !== 0">
        Ontbrekende gemeenten ({{ (ontbrekendeGemeenten?.length || 1) - 1 }})
      </h3>
      <table>
        <tr v-for="g in ontbrekendeGemeenten" :key="g as string">
          <td>{{ g }}</td>
        </tr>
      </table>
    </template>
    <div v-else class="midden">
      <h3>Voortgangsinformatie was nog niet op deze site beschikbaar voor deze verkiezingen.</h3>
    </div>
    <h3 v-if="nietDeelnemendeGemeenten?.length !== 0">
      Gemeenten die niet zelf de gegevens hebben aangeleverd ({{
        (nietDeelnemendeGemeenten?.length || 1) - 1
      }})
    </h3>
    <table>
      <tr v-for="g in nietDeelnemendeGemeenten" :key="g as string">
        <td>{{ g }}</td>
      </tr>
    </table>
    <p v-if="gegevens">
      De getoonde voortgang is vanaf ({{ new Intl.DateTimeFormat().format(gegevens[0]?.datum) }}).
      Vanaf dat moment zijn de gegevens via deze site bijgehouden.
    </p>
  </div>
</template>
